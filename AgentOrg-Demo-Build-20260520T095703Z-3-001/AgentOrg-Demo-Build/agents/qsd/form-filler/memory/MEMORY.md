...
## 2026-08-19（追加）— 客戶回饋：Version 欄一致性可忽略，N/A 列 Procedure number 例外解除

- **前一輪判斷「代碼數與 Version 欄版次數不對應→列為例外保留原樣」是暫時性保守判斷，不是永久規則**：2026-08-19 首次修正時，因 N/A 列（Article 51/53/56/57）代碼數（2）與 Version 欄版次數（1）不對應，依 soul.md 原則 6「Follow the plan」（僅在技術上不可行時偏離，並記錄原因）將這 4 列列為例外、保留原文交客戶確認。客戶事後明確回饋「Version 欄一致性可以忽略」後，這個限制隨即解除，4 列改為可精簡。**教訓**：遇到「規則 A 與規則 B 互相衝突」導致某幾列無法安全套用一般規則時，與其永久排除，不如在回報中清楚點名「衝突的規則是什麼、若客戶豁免其中一條規則即可解除例外」，方便後續客戶一句話回饋就能快速解鎖剩餘修正，不需重新分析整個問題。本次任務因上一輪已把衝突原因寫得很具體（9.4 節逐列列出「代碼數 2 對版次數 1」），這次追加修正只需直接讀原文、重新確認一次現況，即可快速執行，證明「把例外原因記錄得夠具體」對後續任務效率有直接幫助。
- **精簡欄位時，「移除的說明文字」預設應該搬到鄰近的既有欄位，而非直接刪除或另開新欄**：本次「不適用」判定與理由文字整段移入 Version 欄（col6），刻意採「逐字保留搬移」而非重新摘寫兩個理由：(1) 符合 soul.md 原則 1「引用可追溯性」，避免人工改寫過程中意外流失語意或誤譯；(2) 日後若需要追蹤「這段理由文字原本在哪裡、為何搬移」，逐字保留能讓人工複核者直接肉眼比對前後版本，不需要额外猜測是否有資訊被摘要掉。**落點選擇原則**：優先選「與被精簡欄位相鄰、且是列獨有（非跨列 merge）的既有欄位」，例如本例的 col5（Procedure number）相鄰的 col6（Version）；若欄位本身是跨列 merge（如 form-E-Annex 2 的 col3/col4 條款名稱欄），則不可用，因為塞入列特定文字會誤植到共用該 merge 區塊的其他列。
- **同一份 MEMORY.md 中，較早的判斷（如 9.4 節例外列表）在客戶回饋後可能被之後的追加修正取代（superseded），此時不應刪除或覆蓋原記錄，而應在新記錄中明確標註「取代關係」**：本次在 `step5_form_filler_summary.md` 新增「十、追加修正」一節，並在其中寫明「9.4 節列 47/49/52/53 的記錄現已過時，本節為最新狀態」，讓後續讀者（人或 agent）可以從時間序清楚判斷歷史決策的演變過程，而不會誤用舊記錄。此作法可複用於任何「客戶回饋解除先前保守判斷」的追加修正情境。
