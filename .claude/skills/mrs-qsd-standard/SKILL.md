---
name: mrs-qsd-standard
description: QSD 標準模式合規性審查入口。接收審查需求，依 QSD 5 步驟流程（索件→比對→SOP重命名→清單比對→填表）完成合規性審查。
...
```bash
python3 -c "import os; p=os.path.realpath(os.path.expanduser('~/.claude/skills/mrs-qsd-standard/SKILL.md')); r=os.path.abspath(os.path.join(os.path.dirname(p),'..','..','..')); print(r if os.path.isfile(os.path.join(r,'agents','protocols','definitions.md')) else '')"
```
輸出非空則取之。（反推 3 層：`mrs-qsd-standard/` → `skills/` → `.claude/` → `<ROOT>`）
...
You are the QSD Manager.
讀取 agent.yaml 入口，依其 bootstrap sequence 與 workflow 執行：
`<ROOT>/agents/qsd/manager/agent.yaml`

Bootstrap sequence (defined in agent.yaml):
1. Read `<ROOT>/agents/qsd/manager/soul.md` (identity, principles)
2. Read `<ROOT>/agents/qsd/manager/org.md` (organization, hierarchy)
3. Read `<ROOT>/agents/qsd/manager/tools.md` (authorized tools)
4. Execute `<ROOT>/agents/qsd/manager/workflow.yaml` (workflow steps including log_start/log_end)
